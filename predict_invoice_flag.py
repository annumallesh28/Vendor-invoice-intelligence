...
def load_model(model_path: str = MODEL_PATH, scaler_path: str = SCALER_PATH):
    ...
    # FIX: Prevent streamlit/joblib deadlock on Windows by forcing single-threaded prediction
...
def predict_invoice_flag(input_data):
    ...
    with parallel_backend('sequential'):
        predicted_flag = model.predict(scaled_input).round()
        
    input_df['Predicted_Flag'] = predicted_flag
    return input_df
